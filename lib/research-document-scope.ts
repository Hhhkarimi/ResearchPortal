...
const POSITIVE =
  /پژوهش|پژوهشی|پژوهشگر|پژوهانه|گرنت|طرح\s*(?:پژوهشی|تحقیقاتی)|پایان[\s‌-]*نامه|رساله|پروپوزال|اخلاق\s*پژوهش|نشریه\s*علمی|مجله\s*علمی|مقاله\s*علمی|انتشارات\s*علمی|آزمایشگاه|تجهیزات\s*پژوهشی|فناور|نوآور|صنعت|مالکیت\s*فکری|اختراع|دانش[\s‌-]*بنیان|مرکز\s*رشد|پارک\s*علم|فرصت\s*مطالعاتی|پسادکتری|کتابخانه|مرکز\s*اسناد|پایگاه\s*(?:اطلاعاتی|علمی)|راهنمای\s*جستجو|\bspringer\b|\bieee\b|\birandoc\b|\bsid\b|\bscopus\b|\bproquest\b|\bresearch\b|\bthesis\b|\bdissertation\b|\bproposal\b|\bjournal\b|\bpublication\b|\blaborator(?:y|ies)\b|\btechnology\b|\binnovation\b|\bindustry\b|\bpatent\b|\bpostdoc(?:toral)?\b|\bgrant\b/iu;
...
export function classifyPublicResearchDocument(
  ...
  if (HARD_EXCLUDE.test(text)) {
    return {
      keep: false,
      reason: "explicit-non-research",
    };
  }

  const positive =
    POSITIVE.test(text) ||
    POSITIVE_PATH.test(text);

  if (
    SOFT_EXCLUDE.test(text) &&
    !positive
  ) {
    return {
      keep: false,
      reason: "educational-or-student-context",
    };
  }
  ...
  if (!positive && !trusted) {
    return {
      keep: false,
      reason: "research-relevance-unproven",
    };
  }
...
export function isPublicResearchDocument(
  record: any,
  researchRoots: string[] = []
) {
  return classifyPublicResearchDocument(
    record,
    researchRoots
  ).keep;
}
