...
} from "@/lib/document-enrichment";

export const institutions = isc;

const researchRootsBySlug =
...
const rawDocumentRows =
  rawDocs as any[];

const publicDocumentRows =
  rawDocumentRows.filter(
    (row: any) =>
      isPublicResearchDocument(
        row,
        researchRootsBySlug.get(
          row.universitySlug
        ) || []
      )
  );

const excludedDocumentUrlKeys =
  new Set(
    rawDocumentRows
      .filter(
        (row: any) =>
...
const keepEvidenceUrl = (
  url: unknown
) => {
  const key =
    canonicalPublicUrl(url);

  return (
    !key ||
    !excludedDocumentUrlKeys.has(
      key
    )
  );
};
...
export const deepAudits =
  (rawDeepAudit as any[]).map(
    (row) => {
      const dimensions =
        omitInformationTechnologyDimension(
          row.dimensions
...
export const dimensionEvidence =
  (rawDimensions as any[])
    .filter(
      (row) =>
        row.dimension !==
        "informationTechnology"
...
export const researchReviews =
  (rawReviews as any[]).map(
    (review) => {
      const dimensions =
        omitInformationTechnologyDimension(
          review.dimensions
          ...
          (
            ...
            .map(
              (
                ...
                ...source,
                id:
                  `${review.universitySlug}-official-${index}`,
                universitySlug:
...
const reviewCoverageAverage =
  researchReviews.length
    ? Math.round(
        (
          10 *
          researchReviews.reduce(
            (
              sum,
              row: any
            ) =>
