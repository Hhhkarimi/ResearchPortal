...
for(const inst of isc){
 ...
 const packet={schemaVersion:'10.0.0',snapshotDate:'2026-08-11',institution:inst,portalAudit:a,deepAudit:d,dimensionEvidence:dimensionEvidence.filter(x=>x.universitySlug===slug),ranking:r,units:units.filter(x=>x.universitySlug===slug),systems:systems.filter(x=>x.universitySlug===slug),documents:documents.filter(x=>x.universitySlug===slug),interpretation:{iscRank:'ISC classification rank inside the ISC class; independent from RTPMI.',rtpmi:'Portal maturity/transparency only; not research performance.',missing:'unresolved/not-found evidence is not proof of absence and is not automatically scored zero.'}};
